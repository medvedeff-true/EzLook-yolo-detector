...
from PyQt6.QtWidgets import (
    ...
    QSizePolicy, QToolButton, QPushButton, QStyle, QDialog
)
from PyQt6.QtGui import QImage, QPixmap, QIcon, QMovie
from PyQt6.QtCore import QThread, pyqtSignal, Qt, QTimer, QSize, QProcess

# Словари с вариантами моделей YOLO и их описаниями (для удалённой загрузки, если потребуется)
YOLO_VERSIONS = {
    "YOLOv3": "yolov3.pt",
    "YOLOv4": "yolov4.pt",
...
# Обновлённые ссылки для скачивания моделей (если потребуется)
MODEL_URLS = {
...
# --------------------------
# Вспомогательная функция скачивания файла с прогрессом
# --------------------------
...
# ==========================
# КЛАСС ClickableSlider
# ==========================
class ClickableSlider(QSlider):
    """QSlider с возможностью клика в любом месте для мгновенной перемотки."""
...
class VideoThread(QThread):
    ...
    def __init__(self, cap, model, is_file=False, parent=None):
...
# ==========================
# КЛАСС ModelLoaderThread
# ==========================
class ModelLoaderThread(QThread):
    """
    Поток для загрузки модели YOLO.
    Если файл модели существует локально – загружается через YOLO(self.model_file).
    Иначе, попытка удалённой загрузки (например, через torch.hub).
    """
    ...
    def __init__(self, model_file, device, parent=None):
        """
        :param model_file: Имя модели или путь к файлу (например, "yolov8n.pt" или полный путь к локальной модели)
        :param device: Устройство для модели (например, "cpu" или "cuda:0")
        """
        super().__init__(parent)
    ...
    def run(self):
        try:
            ...
            if os.path.exists(self.model_file):
                # Если файл существует локально, загружаем его напрямую
                from ultralytics import YOLO
                model = YOLO(self.model_file)
            else:
                # Если файла нет, пытаемся загрузить удалённо (например, по имени модели)
...
class SettingsDialog(QDialog):
    ...
    def open_download_manager(self):
        ...
        script_path = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "downloading.py"))
...
class SplashScreen(QWidget):
    """
    Файл GIF (loading.gif) должен находиться в папке img.
    """
    def __init__(self):
        super().__init__()
...
# ==========================
# КЛАСС VideoPlayerWindow
# ==========================
class VideoPlayerWindow(QMainWindow):
    """
    Главное окно приложения (плеер).
    """
    def __init__(self):
        super().__init__()
        ...
        # Контролы для выбора файла видео (отображаются для видео файла)
        ...
        try:
            ...
            QMessageBox.critical(self, "Ошибка",
                                 "Отсутствует подключение к интернету, либо произошла ошибка загрузки модели.")
            self.model = None
            self.current_model_version = "Не определена"
    ...
    def start_detection(self):
        ...
        if self.model is None:
            ...
            return
        self.video_thread = VideoThread(cap, self.model, is_file=is_file)
...
if __name__ == '__main__':
    ...
    # Создаём заставку с GIF-анимацией
    splash = SplashScreen()
    splash.show()
